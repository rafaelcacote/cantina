<form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]" novalidate>
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <div class="flex items-center gap-3">
            <div class="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
                @if ($tenant->logoSrc())
                    <img id="tenant-logo-preview" src="{{ $tenant->logoSrc() }}" alt="Logo de {{ $tenant->name }}" class="h-full w-full object-contain">
                @else
                    <img id="tenant-logo-preview" src="" alt="" class="hidden h-full w-full object-contain">
