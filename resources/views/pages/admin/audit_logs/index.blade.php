...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('admin.audit-logs.index') }}" class="grid grid-cols-1 gap-3 lg:grid-cols-7">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('admin.audit-logs.index') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">Limpar</a>
                </div>
            </form>
        </div>
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse ($logs as $log)
                            ...
                            @endphp
                            <tr>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $log->tenant_id ? ($tenantNames[$log->tenant_id] ?? '-') : '-' }}</td>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <a href="{{ route('admin.audit-logs.show', $log) }}" class="rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">Visualizar</a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Nenhum log de auditoria encontrado.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <script>
        (function () {
            ...
            const syncUsers = () => {
                ...
                [...user.options].forEach((opt, idx) => {
                    if (idx === 0) return;
                    const match = !tenantId || opt.dataset.tenantId === tenantId;
                    opt.hidden = !match;
                    opt.disabled = !match;
                });
                if (user.selectedOptions[0]?.disabled) user.value = '';
            };
            tenant.addEventListener('change', syncUsers);
            syncUsers();
        })();
    </script>
@endpush
