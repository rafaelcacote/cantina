...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Responsáveis</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Gerencie responsáveis vinculados aos tenants.</p>
            </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('admin.parents.index') }}" class="mb-4 grid grid-cols-1 gap-3 md:grid-cols-4">
                <input type="text"
                       name="search"
                       value="{{ $search }}"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    @forelse($parents as $parent)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $parent->tenant?->name ?? '-' }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-2">
                                    <a href="{{ route('admin.parents.show', $parent) }}" class="rounded-md px-3 py-1.5 text-xs font-medium text-brand-600 hover:bg-brand-50 dark:text-brand-400 dark:hover:bg-white/5">Visualizar</a>
                                    <a href="{{ route('admin.parents.edit', $parent) }}" class="rounded-md px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-white/5">Editar</a>
                                </div>
                            </td>
                        </tr>
                    @empty
