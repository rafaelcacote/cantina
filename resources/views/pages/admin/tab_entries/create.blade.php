...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-800 dark:text-white/90">Novo Lançamento</h1>
            <a href="{{ route('admin.tab-entries.index') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">Voltar</a>
        </div>
        @if ($errors->any())
        ...
        @endif
        <form method="POST" action="{{ route('admin.tab-entries.store') }}" class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            @csrf
            @include('pages.admin.tab_entries.partials.form')
            <div class="mt-8 flex gap-3">
                ...
                <a href="{{ route('admin.tab-entries.index') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">Cancelar</a>
            </div>
        </form>
    </div>
@endsection
    ...
    <script>
        (function () {
            ...
            tenant.addEventListener('change', () => {
                syncByTenant(student); syncByTenant(tab); syncByTenant(order); syncByTenant(user); syncTabsByStudent();
            });
