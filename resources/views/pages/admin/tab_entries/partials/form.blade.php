    ...
    <div>
        ...
        <select id="tenant_id" name="tenant_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($tenants as $tenant)
                <option value="{{ $tenant->id }}" @selected(old('tenant_id', $entry?->tenant_id) == $tenant->id)>{{ $tenant->name }}</option>
            @endforeach
        </select>
    </div>
    <div>
        ...
        <select id="student_id" name="student_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($students as $student)
                <option value="{{ $student->id }}" data-tenant-id="{{ $student->tenant_id }}" @selected(old('student_id', $entry?->student_id) == $student->id)>{{ $student->name }}</option>
            @endforeach
        </select>
    </div>
    <div>
        ...
        <select id="student_tab_id" name="student_tab_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($studentTabs as $tab)
                <option value="{{ $tab->id }}" data-tenant-id="{{ $tab->tenant_id }}" data-student-id="{{ $tab->student_id }}" @selected(old('student_tab_id', $entry?->student_tab_id) == $tab->id)>#{{ $tab->id }} - Aluno {{ $tab->student_id }}</option>
            @endforeach
        </select>
    </div>
    <div>
        ...
        <select id="order_id" name="order_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($orders as $order)
                <option value="{{ $order->id }}" data-tenant-id="{{ $order->tenant_id }}" @selected(old('order_id', $entry?->order_id) == $order->id)>#{{ $order->id }}</option>
            @endforeach
        </select>
    </div>
    <div>
    ...
    <div class="md:col-span-3">
        ...
        <select id="created_by" name="created_by" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($users as $user)
                <option value="{{ $user->id }}" data-tenant-id="{{ $user->tenant_id }}" @selected(old('created_by', $entry?->created_by) == $user->id)>{{ $user->name }}</option>
            @endforeach
        </select>
    </div>
</div>
