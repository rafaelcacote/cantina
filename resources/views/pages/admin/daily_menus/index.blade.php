...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-800 dark:text-white/90">Cardápios</h1>
            <a href="{{ route('admin.daily-menus.create') }}" class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">
                Novo Cardápio
            </a>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('admin.daily-menus.index') }}" class="grid grid-cols-1 gap-3 lg:grid-cols-5">
                <input
                    type="text"
                    name="search"
                    value="{{ $search }}"
                ...
                <input type="date" name="menu_date" value="{{ $menuDate }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                ...
                <table class="min-w-full">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                        @forelse ($dailyMenus as $dailyMenu)
                            <tr>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $tenantNames[$dailyMenu->tenant_id] ?? '-' }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $dailyMenu->school?->name ?? '-' }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $dailyMenu->menu_date?->format('d/m/Y') }}</td>
                                <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">{{ $dailyMenu->title ?: '-' }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <div class="flex gap-2">
                                        <a href="{{ route('admin.daily-menus.show', $dailyMenu) }}" class="rounded-md border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">
                                            Visualizar
                                        </a>
                                        <a href="{{ route('admin.daily-menus.edit', $dailyMenu) }}" class="rounded-md bg-brand-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-brand-600">
                                            Editar
                                        </a>
                                    </div>
                                </td>
