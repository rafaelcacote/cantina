        ...
        <table class="min-w-full">
            <thead class="bg-gray-50 dark:bg-gray-900/50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">Qtd planejada</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">Qtd disponível</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">Preço</th>
                    <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">Ordem</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                @forelse ($dailyMenu->items->sortBy('sort_order') as $item)
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex rounded-full px-2 py-1 text-xs font-medium {{ $item->active ? 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-300' : 'bg-red-100 text-red-700 dark:bg-red-900/20 dark:text-red-300' }}">
                                {{ $item->active ? 'Ativo' : 'Inativo' }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-sm">
                            <div class="space-y-2">
                                <form method="POST" action="{{ route('admin.daily-menus.items.update', [$dailyMenu, $item]) }}" class="grid grid-cols-1 gap-2 lg:grid-cols-6">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form method="POST" action="{{ route('admin.daily-menus.items.destroy', [$dailyMenu, $item]) }}">
                                    @csrf
                                    @method('DELETE')
