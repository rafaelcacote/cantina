    ...
    <div>
        ...
        <select id="tenant_id" name="tenant_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($tenants as $tenant)
                <option value="{{ $tenant->id }}" @selected(old('tenant_id', $dailyMenu?->tenant_id) == $tenant->id)>
                    {{ $tenant->name }}
                </option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <select id="school_id" name="school_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($schools as $school)
                <option value="{{ $school->id }}" data-tenant-id="{{ $school->tenant_id }}" @selected(old('school_id', $dailyMenu?->school_id) == $school->id)>
                    {{ $school->name }}
                </option>
            @endforeach
        </select>
