...
@section('content')
    <div class="mx-auto max-w-3xl space-y-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Nova Escola</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Cadastre uma nova unidade vinculada a um tenant.</p>
            </div>
        ...
        <form method="POST" action="{{ route('admin.schools.store') }}" class="space-y-5 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            @csrf
            ...
            <div>
                ...
                <textarea name="address" rows="3"
                          class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-brand-300 focus:outline-hidden dark:border-gray-700 dark:text-white/90">{{ old('address') }}</textarea>
            </div>
