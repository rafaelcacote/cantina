...
@section('content')
    <div class="space-y-6">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-800 dark:text-white/90">Editar Produto</h1>
            <a href="{{ route('admin.products.show', $product) }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">
                Voltar
            </a>
        </div>
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            @csrf
            @method('PUT')

            @include('pages.admin.products.partials.form', ['product' => $product])
        ...
        (function () {
            ...
            const syncCategories = () => {
                ...
                [...category.options].forEach((opt, idx) => {
                    if (idx === 0) return;
                    const match = !!tenantId && !!sectionId && opt.dataset.tenantId === tenantId && opt.dataset.sectionId === sectionId;
                    opt.hidden = !match;
                    opt.disabled = !match;
                });
                if (!tenantId || !sectionId || category.selectedOptions[0]?.disabled) category.value = '';
            };
