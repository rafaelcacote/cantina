...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('tenant.daily-menus.index') }}" class="mb-4 flex w-full flex-row gap-3">
                <input type="text"
                       name="search"
                       value="{{ $search }}"
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    ...
                                    <form method="POST" action="{{ route('tenant.daily-menus.destroy', $dailyMenu) }}" onsubmit="return confirm('Excluir este registro?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                title="Excluir"
