...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Detalhes do Cardápio</h1>
        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03] xl:col-span-2">
                ...
                <dl class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div><dt class="text-xs uppercase text-gray-500 dark:text-gray-400">Escola</dt><dd class="mt-1 text-sm font-medium text-gray-800 dark:text-white/90">{{ $dailyMenu->school?->name ?? '-' }}</dd></div>
                    <div><dt class="text-xs uppercase text-gray-500 dark:text-gray-400">Data</dt><dd class="mt-1 text-sm font-medium text-gray-800 dark:text-white/90">{{ $dailyMenu->menu_date?->format('d/m/Y') }}</dd></div>
                    <div><dt class="text-xs uppercase text-gray-500 dark:text-gray-400">Título</dt><dd class="mt-1 text-sm font-medium text-gray-800 dark:text-white/90">{{ $dailyMenu->title ?: '-' }}</dd></div>
                    <div class="md:col-span-2"><dt class="text-xs uppercase text-gray-500 dark:text-gray-400">Descrição</dt><dd class="mt-1 text-sm font-medium text-gray-800 dark:text-white/90">{{ $dailyMenu->description ?: '-' }}</dd></div>
                    <div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <form method="POST" action="{{ route('tenant.daily-menus.items.store', $dailyMenu) }}" class="mt-5 space-y-4">
                    @csrf
                    <div>
