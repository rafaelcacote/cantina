    ...
    <div class="mx-auto max-w-3xl space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Visualize os dados da seção.</p>
            </div>
            <div class="flex flex-wrap items-center gap-2">
                ...
                <form method="POST" action="{{ route('tenant.product-sections.destroy', $section) }}" onsubmit="return confirm('Excluir este registro?')">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            <dl class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div>
                    ...
                    <dd class="mt-1 text-sm font-medium text-gray-800 dark:text-white/90">{{ $section->name }}</dd>
                </div>
                <div>
                    <dt class="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">Slug</dt>
                    <dd class="mt-1 text-sm font-medium text-gray-800 dark:text-white/90">{{ $section->slug ?: '-' }}</dd>
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <dd class="mt-1 text-sm font-medium text-gray-800 dark:text-white/90">{{ $section->description ?: '-' }}</dd>
                </div>
            </dl>
        </div>
    </div>
