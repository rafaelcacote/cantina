...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('tenant.product-sections.index') }}" class="mb-4 flex w-full flex-row gap-3">
                <input type="text"
                       name="search"
                       value="{{ $search }}"
                       placeholder="Buscar por nome ou slug..."
                       class="h-11 min-w-0 flex-1 rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden dark:border-gray-700 dark:text-white/90">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    @forelse($sections as $section)
                        <tr>
                            <td class="px-4 py-3 text-sm font-medium text-gray-800 dark:text-white/90">{{ $section->name }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $section->slug ?: '-' }}</td>
                            <td class="px-4 py-3">
                                <span class="inline-flex rounded-full px-2.5 py-1 text-xs font-medium {{ $section->active ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' }}">
                                    {{ $section->active ? 'Ativo' : 'Inativo' }}
                                </span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    <a href="{{ route('tenant.product-sections.show', $section) }}"
                                       title="Visualizar"
                                    ...
                                    <a href="{{ route('tenant.product-sections.edit', $section) }}"
                                       title="Editar"
                                    ...
                                    <form method="POST" action="{{ route('tenant.product-sections.destroy', $section) }}" onsubmit="return confirm('Excluir este registro?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                title="Excluir"
