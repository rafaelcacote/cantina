...
@section('content')
    @php
        $fromStudent = ! empty($selectedStudentId);
        $fromParent = ! empty($selectedParentId);
        $backUrl = $fromStudent
            ? route('tenant.students.index')
            : ($fromParent
                ? route('tenant.parents.students', $selectedParentId)
                : route('tenant.student-parents.index'));
        $pageTitle = $fromStudent ? 'Adicionar responsável' : ($fromParent ? 'Adicionar aluno' : 'Novo Vínculo');
        $pageSubtitle = $fromStudent
            ? 'Vincule um responsável a este aluno.'
            : ($fromParent ? 'Vincule um aluno a este responsável.' : 'Crie um vínculo entre aluno e responsável.');
    ...
    <div class="mx-auto max-w-3xl space-y-6">
            ...
            <div>
                <h1 class="flex items-center gap-2.5 text-2xl font-semibold text-gray-800 dark:text-white/90">
                    <span class="inline-flex size-8 items-center justify-center text-brand-500 dark:text-brand-400">
                        {!! \App\Helpers\MenuHelper::getIconSvg($pageIcon) !!}
                    </span>
                    {{ $pageTitle }}
                </h1>
        ...
        <form method="POST" action="{{ route('tenant.student-parents.store') }}" class="overflow-hidden rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]" novalidate>
            @csrf
            <div class="space-y-5 p-6">
                @include('pages.tenant.student_parents.partials.form', [
                    'studentParent' => null,
