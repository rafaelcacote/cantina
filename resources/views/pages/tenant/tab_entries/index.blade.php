...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('tenant.tab-entries.create') }}"
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('tenant.tab-entries.index') }}" class="mb-4 flex w-full flex-row flex-wrap gap-3">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    @forelse ($entries as $entry)
                        ...
                        @endphp
                        <tr>
                            <td class="px-4 py-3 text-sm font-medium text-gray-800 dark:text-white/90">{{ $entry->student?->name ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    ...
                                    <form method="POST" action="{{ route('tenant.tab-entries.destroy', $entry) }}" onsubmit="return confirm('Excluir este registro?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                title="Excluir"
