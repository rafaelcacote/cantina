    ...
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Categorias dentro de cada seção de produtos.</p>
            </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('tenant.product-categories.index') }}" class="mb-4 flex w-full flex-row gap-3">
                <input type="text"
                       name="search"
                       value="{{ $search }}"
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    @forelse($categories as $category)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $category->section?->name ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    <a href="{{ route('tenant.product-categories.show', $category) }}"
                                       title="Visualizar"
                                    ...
                                    <form method="POST" action="{{ route('tenant.product-categories.destroy', $category) }}" onsubmit="return confirm('Excluir este registro?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                title="Excluir"
