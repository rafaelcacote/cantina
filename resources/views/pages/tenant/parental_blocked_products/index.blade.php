...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('tenant.parental-blocked-products.create') }}" class="rounded-lg bg-brand-500 px-4 py-2 text-sm font-medium text-white hover:bg-brand-600">Novo Vínculo</a>
        </div>
        @if (session('success'))
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('tenant.parental-blocked-products.index') }}" class="mb-4 flex flex-wrap gap-3">
                ...
                <select name="product_id" class="h-11 min-w-[200px] rounded-lg border border-gray-300 bg-transparent px-4 text-sm dark:border-gray-700 dark:text-white/90">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    <thead><tr class="text-left text-xs uppercase tracking-wide text-gray-500 dark:text-gray-400">
                        <th class="px-4 py-3">Aluno</th><th class="px-4 py-3">Produto</th><th class="px-4 py-3 text-right">Ações</th>
                    </tr></thead>
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex gap-2">
                                    <a href="{{ route('tenant.parental-blocked-products.show', $item) }}" class="rounded-lg border border-gray-300 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300">Visualizar</a>
                                    <a href="{{ route('tenant.parental-blocked-products.edit', $item) }}" class="rounded-lg bg-brand-500 px-3 py-1.5 text-xs font-medium text-white hover:bg-brand-600">Editar</a>
                                </div>
                            </td>
                        </tr>
                    @empty
