...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-white/[0.03]">
            <form method="GET" action="{{ route('tenant.payments.index') }}" class="mb-4 flex w-full flex-row flex-wrap gap-3">
                <select name="student_id"
                        class="h-11 min-w-0 flex-[1.2] basis-0 rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:outline-hidden dark:border-gray-700 dark:text-white/90">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                    ...
                    @forelse ($payments as $payment)
                        ...
                        @endphp
                        <tr>
                            <td class="px-4 py-3 text-sm font-medium text-gray-800 dark:text-white/90">{{ $payment->student?->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $payment->parent?->name ?? '-' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">R$ {{ number_format((float) $payment->amount, 2, ',', '.') }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-300">{{ $payment->paid_at?->format('d/m/Y H:i') ?: '-' }}</td>
                            <td class="px-4 py-3 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    ...
                                    <form method="POST" action="{{ route('tenant.payments.destroy', $payment) }}" onsubmit="return confirm('Excluir este registro?')" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                title="Excluir"
