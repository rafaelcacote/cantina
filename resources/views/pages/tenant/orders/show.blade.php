        ...
        <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
            <div class="xl:col-span-2 rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <dl class="mt-5 grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div><dt class="text-xs uppercase text-gray-500">Registrado por</dt><dd class="mt-1 text-sm text-gray-800 dark:text-gray-200">{{ $order->placedBy?->name ?? '-' }}</dd></div>
                    <div><dt class="text-xs uppercase text-gray-500">Canal</dt><dd class="mt-1 text-sm text-gray-800 dark:text-gray-200">{{ $channels[$order->order_channel] ?? $order->order_channel }}</dd></div>
                    <div><dt class="text-xs uppercase text-gray-500">Tipo</dt><dd class="mt-1 text-sm text-gray-800 dark:text-gray-200">{{ $types[$order->order_type] ?? $order->order_type }}</dd></div>
            ...
            <div class="space-y-6">
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <form method="POST" action="{{ route('tenant.orders.status.update', $order) }}" class="mt-4 space-y-3">
                        @csrf
                        @method('PATCH')
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <form method="POST" action="{{ route('tenant.orders.items.store', $order) }}" class="mt-4 space-y-3">
                        @csrf
                        <select name="product_id" required class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <input type="number" min="1" name="quantity" placeholder="Quantidade" value="1" required class="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                            <input type="number" min="0" step="0.01" name="unit_price" placeholder="Preço unitário (opcional)" class="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
                        </div>
                        <select name="item_status" class="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm focus:border-brand-500 focus:ring-0 dark:border-gray-700 dark:bg-gray-900 dark:text-white">
