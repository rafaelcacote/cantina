...
<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
    <div>
        ...
        <select id="student_id" name="student_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-3 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white">
            ...
            @foreach ($students as $student)
                <option value="{{ $student->id }}" @selected(old('student_id', $tab?->student_id) == $student->id)>{{ $student->name }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        @error('current_balance') <p class="mt-1 text-sm text-red-500">{{ $message }}</p> @enderror
    </div>
    <div>
