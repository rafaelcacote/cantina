...
const props = defineProps({
    ...
    menuHref: { type: String, required: true },
    storeHref: { type: String, required: true },
...
onMounted(() => {
    try {
        cart.value = JSON.parse(sessionStorage.getItem(props.cartKey) || '[]');
    } catch {
        cart.value = [];
    }

    if (!cart.value.length) {
        router.visit(props.menuHref);
        return;
    }
...
const submit = () => {
    form.post(props.storeHref, {
        preserveScroll: true,
        onSuccess: () => {
            sessionStorage.removeItem(props.cartKey);
        },
        onFinish: () => {
    ...
    <MobileShell role="parent">
        <section class="space-y-7 pb-28">
            <div>
                ...
                Revise os itens, escolha como pagar e envie para a cantina em nome de {{ child.name }}.
                </p>
            </div>
        ...
        <ConfirmSheet
            ...
            :message="`O pedido de ${child.name} será pago com ${paymentLabel(form.payment_mode).toLowerCase()}.`"
